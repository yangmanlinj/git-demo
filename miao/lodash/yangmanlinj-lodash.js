...
//nuary返回一个函数，让原函数最多只接受一个参数
  unary: function unary(f) { 
    return function(value) {//返回的函数只接受一个参数
      return f(value)//也只把这一个参数传给原来的函数
    }
  },
  ...
  every: function every(ary, test) { //短路特性
    for (var i = 0; i < ary.length; i++) {
      if (!test(ary[i], i, ary)) {
        return false
      }
